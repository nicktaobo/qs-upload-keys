...
async function api(request, response) {
  ...
  try {
    ...
    if (route === "/api/metrics" && method === "GET") { const user = await requireUser(request, response); if (!user) return; if (user.role === "ADMIN") { const result = await database.query("select rpm,tpm,sampled_at from platform_metrics where id=true"); const metric = result.rows[0] || { rpm: 0, tpm: 0 }; return json(response, 200, { rpm: Number(metric.rpm), tpm: Number(metric.tpm), sampledAt: metric.sampled_at, source: "upstream" }); } const result = await database.query("select coalesce(sum(rpm),0)::int as rpm,coalesce(sum(tpm),0)::bigint as tpm,max(last_synced_at) as sampled_at from api_keys where owner_id=$1", [user.id]); const metric = result.rows[0] || { rpm: 0, tpm: 0 }; return json(response, 200, { rpm: Number(metric.rpm), tpm: Number(metric.tpm), sampledAt: metric.sampled_at, source: "user_keys" }); }
    if (route === "/api/metrics/refresh" && method === "POST") { const user = await requireUser(request, response); if (!user) return; if (user.role === "ADMIN") return json(response, 200, await refreshUpstreamMetrics()); const result = await database.query("select coalesce(sum(rpm),0)::int as rpm,coalesce(sum(tpm),0)::bigint as tpm,max(last_synced_at) as sampled_at from api_keys where owner_id=$1", [user.id]); const metric = result.rows[0] || { rpm: 0, tpm: 0 }; return json(response, 200, { rpm: Number(metric.rpm), tpm: Number(metric.tpm), sampledAt: metric.sampled_at, source: "user_keys" }); }
