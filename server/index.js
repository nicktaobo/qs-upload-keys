...
const sanitizeUser = (user) => ({
  ...
  channelTemplate: user.role === "SUPPLIER" && user.channel_type_id && user.channel_group && user.channel_name_prefix ? {
    ...
    models: Array.isArray(user.channel_models) ? user.channel_models : []
  } : null
});
...
async function syncUpstreamMetadata() {
  ...
  await database.query("update app_users set channel_models=coalesce($1::jsonb -> channel_type_id::text,'[]'::jsonb) where role='SUPPLIER' and jsonb_array_length(channel_models)=0", [JSON.stringify(typeModels)]);
...
function resolveChannelTemplate(metadata, body) {
  ...
  const requestedModels = uniqueStrings(body.channelModels);
  const models = requestedModels.length ? requestedModels : allowedModels;
  if (!channelType || !metadata.groups.includes(group) || !prefix || prefix.length > 80 || !models.length || models.some((model) => !allowedModels.includes(model))) return null;
  return { typeId, group, prefix, models, channelType };
}
...
async function api(request, response) {
  ...
  try {
    ...
    if (route === "/api/admin/users" && method === "GET") { const user = await requireUser(request, response, true); if (!user) return; const result = await database.query("select u.id,u.username,u.role,u.channel_type_id,u.channel_group,u.channel_name_prefix,u.channel_models,u.is_active,u.created_at,count(k.id)::int as keys from app_users u left join api_keys k on k.owner_id=u.id group by u.id order by u.created_at asc"); return json(response, 200, { items: result.rows.map((item) => ({ ...item, role: item.role === "ADMIN" ? "管理员" : "供应商", status: item.is_active ? "正常" : "已停用", channelTemplate: item.role === "SUPPLIER" && item.channel_type_id && item.channel_group && item.channel_name_prefix ? { typeId: Number(item.channel_type_id), group: item.channel_group, prefix: item.channel_name_prefix, models: Array.isArray(item.channel_models) ? item.channel_models : [] } : null })) }); }
    if (route === "/api/admin/users" && method === "POST") { const user = await requireUser(request, response, true); if (!user) return; const body = await parseBody(request); const username = String(body.username || "").trim(); if (!username || String(body.password || "").length < 8) return json(response, 400, { message: "用户名不能为空，密码至少需要 8 位" }); const role = body.role === "管理员" ? "ADMIN" : "SUPPLIER"; const metadata = role === "SUPPLIER" ? await getCachedMetadata() : null; const template = role === "SUPPLIER" ? resolveChannelTemplate(metadata, body) : null; if (role === "SUPPLIER" && !template) return json(response, 400, { message: "请选择有效的渠道类型、分组和模型，并填写不超过 80 个字符的渠道名前缀" }); const result = await database.query("insert into app_users (username,password_hash,role,channel_type_id,channel_group,channel_name_prefix,channel_models) values ($1,$2,$3,$4,$5,$6,$7) returning *", [username, hashPassword(String(body.password)), role, template?.typeId || null, template?.group || null, template?.prefix || null, JSON.stringify(template?.models || [])]).catch((error) => error.code === "23505" ? null : Promise.reject(error)); if (!result) return json(response, 409, { message: "用户名已存在" }); return json(response, 201, { user: sanitizeUser(result.rows[0]) }); }
    if (route.startsWith("/api/admin/users/") && route.endsWith("/template") && method === "POST") { const user = await requireUser(request, response, true); if (!user) return; const id = route.split("/")[4]; const body = await parseBody(request); const metadata = await getCachedMetadata(); const template = resolveChannelTemplate(metadata, body); if (!template) return json(response, 400, { message: "请选择有效的渠道类型、分组和模型，并填写不超过 80 个字符的渠道名前缀" }); const result = await database.query("update app_users set channel_type_id=$1,channel_group=$2,channel_name_prefix=$3,channel_models=$4 where id=$5 and role='SUPPLIER' returning *", [template.typeId, template.group, template.prefix, JSON.stringify(template.models), id]); if (!result.rowCount) return json(response, 404, { message: "供应商用户不存在" }); return json(response, 200, { user: sanitizeUser(result.rows[0]) }); }
    ...
    if (route === "/api/channels" && method === "GET") { const user = await requireUser(request, response); if (!user) return; const result = await database.query("select c.*, count(k.id)::int as key_count, coalesce(sum(k.usage_usd),0)::float as usage_usd, array_remove(array_agg(distinct k.status),null) as key_statuses, array_remove(array_agg(distinct k.upstream_channel_id),null) as upstream_channel_ids from channel_configs c left join api_keys k on k.channel_config_id=c.id where c.owner_id=$1 group by c.id order by c.created_at desc", [user.id]); return json(response, 200, { items: result.rows.map((item) => ({ id: item.id, name: item.name, provider: item.provider, models: item.models, group: item.group_name, discount: String(item.discount), state: item.key_statuses?.includes("UPSTREAM_ERROR") ? "同步异常" : item.key_statuses?.every((status) => status === "SYNCED") ? "已同步" : item.key_statuses?.includes("PENDING") ? "同步中" : "模拟记录", keyCount: item.key_count, upstreamChannelIds: item.upstream_channel_ids || [], usageUsd: item.usage_usd, createdAt: item.created_at })) }); }
    if (route === "/api/keys" && method === "GET") { const user = await requireUser(request, response); if (!user) return; const result = await database.query("select k.id,k.key_name,k.status,k.upstream_channel_id,k.usage_usd,k.rpm,k.tpm,c.name as channel_name,c.models,c.group_name from api_keys k join channel_configs c on c.id=k.channel_config_id where k.owner_id=$1 order by k.created_at desc", [user.id]); return json(response, 200, { items: result.rows.map((item) => ({ id: item.id, name: item.key_name, channel: item.channel_name, upstreamChannelId: item.upstream_channel_id, model: item.models?.[0] || "未指定", group: item.group_name, usageUsd: Number(item.usage_usd), rpm: item.rpm, tpm: item.tpm, status: item.status })) }); }
    ...
    if (route === "/api/channels" && method === "POST") {
      ...
      const allowedModels = metadata?.typeModels[String(typeId)]?.length ? metadata.typeModels[String(typeId)] : metadata?.enabledModels || [];
      const requestedModels = uniqueStrings(body.models);
      const configuredModels = uniqueStrings(user.channel_models).filter((model) => allowedModels.includes(model));
      const models = isSupplier ? (requestedModels.length ? requestedModels : (typeId === Number(user.channel_type_id) && configuredModels.length ? configuredModels : allowedModels)) : requestedModels;
