...
const state = {
  ...
  page: "overview", modal: false, userModal: false, batch: false, mappings: [], usageUpdated: "09:09:40",
  ...
  channelDraft: { typeId: "", name: "", discount: "1.000", keys: "", group: "default", models: null, autoDisable: true }
};
...
const newChannelDraft = () => ({ typeId: String(state.metadata.channelTypes[0]?.id || ""), name: "", discount: "1.000", keys: "", group: state.metadata.groups.includes("default") ? "default" : (state.metadata.groups[0] || ""), models: null, autoDisable: true });
...
function render() {
  ...
  app.innerHTML = `<div class="shell">
    <aside class="sidebar"><div class="brand"><span class="brand-mark">${icon("key")}</span><span>渠道中转台</span><small>QUSHU RELAY</small></div>
...
function renderPage() {
  ...
  return `<section class="content"><div class="intro"><div><p class="eyebrow">SUPPLIER CONSOLE / ${isOverview ? "01" : "02"}</p><h1>${isOverview ? "平台概览" : "渠道管理"}</h1><p class="subline">${isOverview ? "查看平台运行状态、吞吐能力和渠道健康度。" : "使用上游实时配置创建本地模拟渠道。"}</p></div><div class="intro-actions">${state.user?.role === "admin" ? `<button class="secondary-button" id="refresh-metadata">${icon("refresh")} 同步配置数据</button>` : ""}<button class="primary-button" id="open-modal">${icon("plus")} 添加渠道</button></div></div><div class="metadata-strip"><span class="status-dot"></span><span>渠道类型 ${state.metadata.channelTypes.length} 种 · 分组 ${state.metadata.groups.length} 个 · 模型 ${state.metadata.allModels.length} 个</span><time>上次同步：${esc(formatTime(state.metadata.syncedAt))}</time></div>
...
function myKeysPage() {
  ...
  const myRpm = records.reduce((sum, item) => sum + item.rpm, 0);
  const myTpm = records.reduce((sum, item) => sum + item.tpm, 0);
  return `<section class="content"><div class="page-band"><div><p class="eyebrow">PERSONAL KEYS / 01</p><h1>我的 Key</h1><p class="subline">只展示当前用户自己提交的 Key 名称与用量，不显示完整密钥。</p></div><button class="primary-button" data-page="channels">${icon("plus")} 提交新 Key</button></div><div class="user-context"><span class="avatar small-avatar">${esc((state.user?.username || "U").slice(0, 2).toUpperCase())}</span><div><strong>${esc(state.user?.username || "本地演示")}</strong><small>${state.user?.role === "admin" ? "管理员" : "供应商用户"} · 仅可查看本人数据</small></div><span class="context-lock">${icon("lock")} 数据已隔离</span></div><div class="stats user-stats">${stat("我的 Key", records.length, "枚凭据")}${stat("今日用量", `$${myAmount.toFixed(3)}`, "累计消费")}${stat("我的 RPM", myRpm.toLocaleString(), "请求 / 分钟", "metric-live")}${stat("我的 TPM", myTpm.toLocaleString(), "Token / 分钟", "metric-live")}</div><div class="section-head"><div><h2>Key 使用明细</h2><p>Key 名称由平台生成，完整密钥仅在服务端加密保存。</p></div><button class="text-button" id="refresh-my-usage">${icon("refresh")} 刷新用量</button></div><div class="table-wrap key-usage-table"><table><thead><tr><th>Key 名称</th><th>所属渠道</th><th>模型</th><th>分组</th><th>使用量</th><th>RPM</th><th>TPM</th><th>状态</th></tr></thead><tbody>${records.map((item) => `<tr><td><div class="key-label"><span>${icon("key")}</span><strong>${esc(item.name)}</strong></div></td><td>${esc(item.channel)}</td><td class="mono">${esc(item.model)}</td><td><span class="group-tag">${esc(item.group)}</span></td><td class="money">$${item.amount.toFixed(3)}</td><td class="mono">${item.rpm.toLocaleString()}</td><td class="mono">${item.tpm.toLocaleString()}</td><td><span class="sync-state"><i></i>${esc(item.state)}</span></td></tr>`).join("")}</tbody></table>${records.length === 0 ? `<div class="empty">你还没有提交 Key。</div>` : ""}</div></section>`;
}

function usersPage() {
  return `<section class="content"><div class="page-band"><div><p class="eyebrow">ACCESS CONTROL / 04</p><h1>用户管理</h1><p class="subline">平台账号由管理员创建和停用，普通用户不能自行注册。</p></div><button class="primary-button" id="open-user-modal">${icon("plus")} 创建用户</button></div><div class="permission-strip"><span class="permission-icon">${icon("lock")}</span><div><strong>管理员专属</strong><p>只有管理员可以创建、停用和重置平台用户；用户只会看到被授权的渠道和统计。</p></div></div><div class="table-wrap user-table"><table><thead><tr><th>用户名</th><th>角色</th><th>状态</th><th>已接入 Key</th><th>创建时间</th><th>操作</th></tr></thead><tbody>${state.users.map((user) => `<tr><td><div class="user-cell"><span class="avatar small-avatar">${esc(user.username.slice(0, 2).toUpperCase())}</span><strong>${esc(user.username)}</strong></div></td><td><span class="role-tag">${esc(user.role)}</span></td><td><span class="status-badge ${user.status === "正常" ? "success" : "danger"}">${esc(user.status)}</span></td><td>${user.keys}</td><td class="mono">${esc(user.createdAt)}</td><td><button class="row-action" data-toggle-user="${esc(user.id)}" data-user-active="${user.status === "正常"}" ${user.username === state.user?.username ? "disabled" : ""}>${user.status === "正常" ? "停用" : "启用"}</button></td></tr>`).join("")}</tbody></table></div></section>`;
}
...
function channelModal() {
  ...
  const typeId = String(state.channelDraft.typeId || types[0]?.id || "");
  ...
  return `<div class="modal-backdrop" id="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div class="modal-head"><div><p class="eyebrow">NEW CHANNEL / SIMULATION</p><h2 id="modal-title">添加渠道</h2></div><button class="close-button" id="close-modal" title="关闭">${icon("close")}</button></div><div class="modal-scroll"><div class="callout simulation-callout"><span>${icon("lock")}</span><div><strong>当前为模拟提交</strong><small>类型、模型和分组来自上游实时配置；本次只保存到本平台，不会写入上游</small></div><time>${esc(formatTime(state.metadata.syncedAt))}</time></div>${types.length ? `<div class="form-grid"><label class="field full"><span>类型 <b>*</b><small class="field-help">${esc(currentType?.authMethods?.join(" / ") || "上游同步")}</small></span><select id="provider">${types.map((item) => `<option value="${item.id}" ${String(item.id) === typeId ? "selected" : ""}>${esc(item.name)}</option>`).join("")}</select></label><label class="field"><span>折扣 <b>*</b></span><div class="number-input"><button type="button" data-step="-0.01">−</button><input id="discount" type="number" step="0.001" min="0" max="10" value="${esc(state.channelDraft.discount)}"><button type="button" data-step="0.01">+</button></div></label><label class="field"><span>渠道名称 <b>*</b></span><input id="channel-name" value="${esc(state.channelDraft.name)}" placeholder="例如：production-openai"></label><label class="field full"><span>Key <b>*</b><small class="field-help">每行一个，开启批量添加后可一次提交多枚</small></span><textarea id="keys" rows="${state.batch ? 4 : 2}" placeholder="请输入渠道对应的鉴权密钥">${esc(state.channelDraft.keys)}</textarea><label class="toggle-line"><input id="batch" type="checkbox" ${state.batch ? "checked" : ""}><span>批量添加</span><small id="key-counter">${state.batch ? `${keyCount} 枚待提交` : "单枚 Key"}</small></label></label><label class="field"><span>分组 <b>*</b></span><select id="group">${state.metadata.groups.map((group) => `<option value="${esc(group)}" ${group === state.channelDraft.group ? "selected" : ""}>${esc(group)}</option>`).join("")}</select></label><label class="field"><span>当前可选模型</span><div class="field-readout">${models.length} 个</div></label><label class="field full"><span>模型 <b>*</b><small class="field-help">按当前渠道类型自动筛选</small></span><div class="chips model-picker" id="model-chips">${models.map((model) => `<button type="button" class="chip ${selectedModels.includes(model) ? "selected" : ""}" data-model="${esc(model)}">${esc(model)} <span>×</span></button>`).join("") || `<span class="empty-inline">该类型没有可用模型，请先同步配置数据</span>`}</div><button type="button" class="outline-button" id="fill-models">${icon("check")} 全选当前模型</button></label><div class="field full"><span>高级设置</span><div class="advanced"><label class="toggle-line"><input id="auto-disable" type="checkbox" ${state.channelDraft.autoDisable ? "checked" : ""}><span>自动禁用</span><small>后续真实同步时，测试失败自动停用</small></label><div id="mapping-list">${state.mappings.map((item, index) => mappingRow(index, item)).join("")}</div><button type="button" class="text-button" id="add-mapping">${icon("plus")} 添加模型重定向</button></div></div></div>` : `<div class="empty modal-empty">还没有可用配置，请由管理员先同步配置数据。</div>`}</div><div class="modal-foot"><span><i class="secure-dot"></i> Key 加密存入本平台数据库</span><div><button class="secondary-button" id="cancel-modal">取消</button><button class="primary-button" id="submit-channel" ${types.length ? "" : "disabled"}>${icon("arrow")} 模拟提交</button></div></div></section></div>`;
}

function userModal() { return `<div class="modal-backdrop" id="user-modal-backdrop"><section class="modal small-modal" role="dialog" aria-modal="true"><div class="modal-head"><div><p class="eyebrow">ADMIN ONLY / ACCOUNT</p><h2>创建平台用户</h2></div><button class="close-button" id="close-user-modal" title="关闭">${icon("close")}</button></div><div class="modal-scroll"><div class="callout"><span>${icon("users")}</span><div><strong>公开注册已关闭</strong><small>新用户仅可由当前管理员创建</small></div></div><div class="form-grid"><label class="field full"><span>用户名 <b>*</b></span><input id="new-username" placeholder="例如：supplier-001"></label><label class="field full"><span>初始密码 <b>*</b></span><input id="new-password" type="password" placeholder="至少 8 位"></label><label class="field full"><span>角色</span><select id="new-role"><option>供应商</option><option>管理员</option></select></label></div></div><div class="modal-foot"><span><i class="secure-dot"></i> 账号创建后可再重置密码</span><div><button class="secondary-button" id="cancel-user-modal">取消</button><button class="primary-button" id="submit-user">${icon("check")} 创建用户</button></div></div></section></div>`; }
...
function bindEvents() {
  ...
  document.querySelector("#open-user-modal")?.addEventListener("click", () => { state.userModal = true; render(); });
...
async function login(event) { event.preventDefault(); state.loginError = ""; try { const payload = await apiRequest("/api/auth/login", { method: "POST", body: JSON.stringify({ username: document.querySelector("#login-username").value, password: document.querySelector("#login-password").value }) }); state.user = payload.user; await loadRemoteData(); render(); } catch (error) { state.loginError = error instanceof Error ? error.message : "登录失败"; render(); } }
...
function closeUserModal() { state.userModal = false; render(); }
...
async function submitChannel() { const name = document.querySelector("#channel-name").value.trim(); const keys = document.querySelector("#keys").value.split(/\n+/).map((x) => x.trim()).filter(Boolean); const models = [...document.querySelectorAll(".chip.selected")].map((chip) => chip.dataset.model); if (!name || !keys.length || !models.length) { toast("请填写渠道名称、Key，并至少选择一个模型"); return; } const typeId = Number(document.querySelector("#provider").value); const channelType = state.metadata.channelTypes.find((item) => item.id === typeId); const modelMapping = Object.fromEntries(state.mappings.map((_, index) => [document.querySelector(`[data-map-from="${index}"]`)?.value.trim(), document.querySelector(`[data-map-to="${index}"]`)?.value.trim()]).filter(([from, to]) => from && to)); if (state.apiMode) { try { const result = await apiRequest("/api/channels", { method: "POST", body: JSON.stringify({ name, typeId, models, keys, group: document.querySelector("#group").value, discount: Number(document.querySelector("#discount").value), autoDisable: document.querySelector("#auto-disable").checked, modelMapping }) }); await loadRemoteData(); closeModal(); toast(result.upstreamSubmitted ? "渠道已同步" : "模拟提交成功，未写入上游"); } catch (error) { toast(error instanceof Error ? error.message : "模拟提交失败"); } return; } const channel = { id: `local-${Date.now().toString().slice(-5)}`, name, provider: channelType?.name || "未知类型", models, group: document.querySelector("#group").value || "default", discount: Number(document.querySelector("#discount").value).toFixed(3).replace(/0+$/, "").replace(/\.$/, ""), state: "模拟记录", keyCount: keys.length, updatedAt: "刚刚", amount: 0, latency: 0, createdAt: new Date().toLocaleString("zh-CN", { hour12: false }) }; state.channels.unshift(channel); localStorage.setItem("qushu-channels", JSON.stringify(state.channels)); closeModal(); render(); toast("模拟提交成功，未写入上游"); }
async function submitUser() { const username = document.querySelector("#new-username").value.trim(); const password = document.querySelector("#new-password").value; if (!username || password.length < 8) { toast("用户名不能为空，密码至少需要 8 位"); return; } if (state.apiMode) { try { await apiRequest("/api/admin/users", { method: "POST", body: JSON.stringify({ username, password, role: document.querySelector("#new-role").value }) }); await loadRemoteData(); closeUserModal(); toast("用户创建成功"); } catch (error) { toast(error instanceof Error ? error.message : "创建失败"); } return; } if (state.users.some((user) => user.username === username)) { toast("用户名已存在"); return; } state.users.push({ username, role: document.querySelector("#new-role").value, status: "正常", createdAt: new Date().toLocaleString("zh-CN", { hour12: false }), keys: 0 }); localStorage.setItem("qushu-users", JSON.stringify(state.users)); closeUserModal(); toast("用户创建成功"); }
...
async function bootstrap() { try { state.user = (await apiRequest("/api/auth/me")).user; await loadRemoteData(); } catch { state.user = null; } finally { state.booting = false; render(); } }
