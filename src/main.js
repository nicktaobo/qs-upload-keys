...
function bindEvents() {
  ...
  document.querySelector("#refresh-my-usage")?.addEventListener("click", refreshUsage);
...
async function refreshTraffic() { if (state.apiMode) { try { const metrics = state.user?.role === "admin" ? await apiRequest("/api/metrics/refresh", { method: "POST", body: "{}" }) : await apiRequest("/api/metrics"); state.traffic = { rpm: Number(metrics.rpm || 0), tpm: Number(metrics.tpm || 0) }; state.usageUpdated = metrics.sampledAt ? new Date(metrics.sampledAt).toLocaleTimeString("zh-CN", { hour12: false }) : state.usageUpdated; render(); toast(state.user?.role === "admin" ? "上游 RPM / TPM 已刷新" : "平台 RPM / TPM 已更新"); } catch (error) { toast(error instanceof Error ? error.message : "刷新失败"); } return; } state.traffic.rpm = Math.round(state.traffic.rpm * (0.96 + Math.random() * 0.08)); state.traffic.tpm = Math.round(state.traffic.tpm * (0.96 + Math.random() * 0.08)); state.usageUpdated = new Date().toLocaleTimeString("zh-CN", { hour12: false }); render(); toast("RPM / TPM 已刷新"); }
