...
function channelTable(compact = false) {
  return `<div class="table-wrap"><table><thead><tr><th>渠道名称</th><th>上游渠道 ID</th><th>类型</th><th>模型</th><th>分组</th><th>折扣</th><th>状态</th><th>操作</th></tr></thead><tbody>${state.channels.map((channel) => `<tr><td><div class="channel-name"><span class="provider-icon">${channel.provider.slice(0, 1)}</span><div><strong>${esc(channel.name)}</strong><small>内部编号 ${esc(channel.id || "待分配")} · ${esc(channel.updatedAt)}</small></div></div></td><td>${channel.upstreamChannelIds?.length ? `<div class="upstream-id-list">${channel.upstreamChannelIds.map((id) => `<span>${esc(id)}</span>`).join("")}</div>` : `<span class="upstream-id-empty">${channel.state === "模拟记录" ? "未生成" : "待回填"}</span>`}</td><td>${esc(channel.provider)}</td><td><span class="model-count">${channel.models.length} 个模型</span><div class="model-preview">${channel.models.slice(0, 2).map(esc).join(" · ")}</div></td><td><span class="group-tag">${esc(channel.group || "default")}</span></td><td><strong class="discount">${esc(channel.discount)}</strong></td><td><span class="sync-state"><i></i>${esc(channel.state)}</span></td><td><button class="row-action" data-page="usage" title="查看使用统计">${icon("arrow")}</button></td></tr>`).join("")}</tbody></table>${state.channels.length === 0 ? `<div class="empty">还没有渠道，点击右上角添加第一条配置。</div>` : ""}</div>${compact ? "" : `<div class="notice"><span class="notice-icon">${icon("lock")}</span><div><strong>敏感数据隔离</strong><p>上游地址、请求参数、鉴权信息和原始响应只在服务端处理，用户界面不会展示。</p></div></div>`}`;
}
...
function myKeysPage() {
  ...
  const records = state.apiMode ? state.keys.map((item) => ({ name: item.name, channel: item.channel, upstreamChannelId: item.upstreamChannelId, model: item.model, group: item.group, amount: Number(item.usageUsd || 0), rpm: Number(item.rpm || 0), tpm: Number(item.tpm || 0), state: item.status === "UPSTREAM_ERROR" ? "异常" : item.status === "SIMULATED" ? "模拟记录" : item.status === "PENDING" ? "同步中" : "正常" })) : demoRecords;
  const myAmount = records.reduce((sum, item) => sum + item.amount, 0);
  return `<section class="content"><div class="page-band"><div><p class="eyebrow">PERSONAL KEYS / 01</p><h1>我的 Key</h1><p class="subline">只展示当前用户自己提交的 Key 名称与用量，不显示完整密钥。</p></div><button class="primary-button" data-page="channels">${icon("plus")} 提交新 Key</button></div><div class="user-context"><span class="avatar small-avatar">${esc((state.user?.username || "U").slice(0, 2).toUpperCase())}</span><div><strong>${esc(state.user?.username || "本地演示")}</strong><small>${state.user?.role === "admin" ? "管理员" : "供应商用户"} · Key 数据仅本人可见，平台吞吐由管理员共享</small></div><span class="context-lock">${icon("lock")} 数据已隔离</span></div><div class="stats user-stats">${stat("我的 Key", records.length, "枚凭据")}${stat("今日用量", `$${myAmount.toFixed(3)}`, "累计消费")}${stat("平台 RPM", state.traffic.rpm.toLocaleString(), `请求 / 分钟 · ${state.usageUpdated}`, "metric-live")}${stat("平台 TPM", state.traffic.tpm.toLocaleString(), `Token / 分钟 · ${state.usageUpdated}`, "metric-live")}</div><div class="section-head"><div><h2>Key 使用明细</h2><p>每枚 Key 对应一个上游渠道 ID；完整密钥仅在服务端加密保存。</p></div><button class="text-button" id="refresh-my-usage">${icon("refresh")} 刷新显示</button></div><div class="table-wrap key-usage-table"><table><thead><tr><th>Key 名称</th><th>上游渠道 ID</th><th>所属渠道</th><th>模型</th><th>分组</th><th>使用量</th><th>RPM</th><th>TPM</th><th>状态</th></tr></thead><tbody>${records.map((item) => `<tr><td><div class="key-label"><span>${icon("key")}</span><strong>${esc(item.name)}</strong></div></td><td class="mono upstream-id">${item.upstreamChannelId ? esc(item.upstreamChannelId) : `<span class="upstream-id-empty">${item.state === "模拟记录" ? "未生成" : "待回填"}</span>`}</td><td>${esc(item.channel)}</td><td class="mono">${esc(item.model)}</td><td><span class="group-tag">${esc(item.group)}</span></td><td class="money">$${item.amount.toFixed(3)}</td><td class="mono">${item.rpm.toLocaleString()}</td><td class="mono">${item.tpm.toLocaleString()}</td><td><span class="sync-state"><i></i>${esc(item.state)}</span></td></tr>`).join("")}</tbody></table>${records.length === 0 ? `<div class="empty">你还没有提交 Key。</div>` : ""}</div></section>`;
}
